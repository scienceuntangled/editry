...
#'
#' @export
er_install_ffmpeg <- function(force = FALSE, check_hash = TRUE) {
    ...
    if (my_os != "windows" || !isTRUE(.Machine$sizeof.pointer == 8)) {
        stop("er_install_ffmpeg only supports 64-bit windows platforms. You will need to install ffmpeg yourself and ensure that it is on the system path.")
    }
